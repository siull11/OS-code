...
int main(int argc, char* argv[]) {
    ...
    // Do work
    double max = vec[0];
    for (unsigned long i = 1; i < vec.size(); i++) if (vec[i] > max) max = vec[i];

    // Gather results
    if (me == ROOT) vec.resize(np);
    My_MPI_Gather((void*) &max, 1, MPI_DOUBLE, &vec[0], 1, MPI_DOUBLE, ROOT, MPI_COMM_WORLD);
